...
export default async function InvitesPage() {
  ...
  if (isBrand) {
    ...
    const { data: invites } = brand
      ? await admin
          ...
          .select(
            '*, campaigns(title), creator_profiles(id, users(display_name, avatar_url))'
          )
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Every creator you&apos;ve reached out to, and whether they&apos;ve
            said yes.
          </p>
        </div>
  ...
  const { data: invites } = creator
    ? await adminClient
        .from('campaign_invites')
        .select(
          '*, campaigns(id, title, brief), brand_profiles(company_name, logo_url)'
        )
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {boostUiEnabled() && creator?.onboarding_completed_at && (
        <BoostHint
          boostUntil={creator?.boost_active_until ?? null}
