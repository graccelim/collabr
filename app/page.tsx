...
const Arrow = () => <svg {...ico} width={16} height={16} strokeWidth={2.5}><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
const Check = () => <svg {...ico} width={15} height={15}><polyline points="20 6 9 17 4 12" /></svg>
const Target = () => <svg {...ico}><circle cx="12" cy="12" r="9" /><circle cx="12" cy="12" r="5" /><circle cx="12" cy="12" r="1" /></svg>
const Send = () => <svg {...ico}><path d="M22 2 11 13" /><path d="M22 2 15 22l-4-9-9-4Z" /></svg>
const Filter = () => <svg {...ico}><path d="M22 3H2l8 9.5V19l4 2v-8.5L22 3z" /></svg>
const Badge = () => <svg {...ico}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /><polyline points="9 12 11 14 15 10" /></svg>
const Lock = () => <svg {...ico}><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
const Star = () => <svg {...ico}><polygon points="12 2 15.1 8.6 22 9.3 17 14 18.2 21 12 17.6 5.8 21 7 14 2 9.3 8.9 8.6 12 2" /></svg>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)', fontFamily: 'var(--font-body)' }}>
      ...
      <nav style={{
        ...
        background: 'rgba(253,250,249,.85)', backdropFilter: 'blur(10px)',
      ...
      <header style={{ textAlign: 'center', padding: 'clamp(36px,6vw,64px) 20px clamp(24px,3vw,32px)', maxWidth: 860, margin: '0 auto' }}>
        <Reveal immediate>
          <Link href="/signup" className="badge" style={{ marginBottom: 18, background: 'var(--surface-2)', color: 'var(--ink-soft)', textDecoration: 'none' }}>
            <span style={{ color: 'var(--creator-deep)' }}>✦</span> Free during beta · No credit card
          </Link>
          <h1 className="display-face" style={{ fontSize: 'clamp(32px,5.2vw,58px)', lineHeight: 1.02, letterSpacing: '-0.03em', marginBottom: 16 }}>
            Collaborations matched<br />by fit, not follower count.
          </h1>
          <p style={{ color: 'var(--ink-soft)', fontSize: 'clamp(15px,2vw,18px)', maxWidth: 540, margin: '0 auto', lineHeight: 1.5 }}>
            Brands find creators. Creators find campaigns. Recommended by niche, audience and budget — no cold outreach, no endless scrolling.
          </p>
        </Reveal>
      </header>
      ...
      <div className="split-hero">
        <Link href="/signup?role=brand" className="split-side split-side-brand" style={{ textDecoration: 'none' }}>
          ...
          <Reveal immediate x={-40} duration={0.6} delay={0.06} style={{ position: 'relative', zIndex: 1, maxWidth: 440 }}>
            <span className="badge" style={{ marginBottom: 18, background: 'rgba(255,255,255,.12)', color: '#fff' }}>For brands</span>
            <h2 style={{ fontSize: 'clamp(26px,3.2vw,40px)', color: '#fff', marginBottom: 18, lineHeight: 1.06, letterSpacing: '-0.02em' }}>
            ...
            <span className="btn btn-lg hover-lift" style={{ background: '#fff', color: 'var(--ink)', display: 'inline-flex', gap: 8 }}>
              Find creators <Arrow />
            </span>
          </Reveal>
        </Link>
        ...
        <Link href="/signup?role=creator" className="split-side split-side-creator" style={{ textDecoration: 'none' }}>
          ...
          <Reveal immediate x={40} duration={0.6} delay={0.16} style={{ position: 'relative', zIndex: 1, maxWidth: 440, marginLeft: 'auto' }}>
            <span className="badge" style={{ marginBottom: 18, background: 'rgba(28,25,23,.08)', color: 'var(--ink)' }}>For creators</span>
            <h2 style={{ fontSize: 'clamp(26px,3.2vw,40px)', color: 'var(--ink)', marginBottom: 18, lineHeight: 1.06, letterSpacing: '-0.02em' }}>
              Find the right campaigns.
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 11, marginBottom: 30 }}>
              {['Matched to your niche and rate', 'Get invited by brands, get paid securely'].map(t => (
                <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 9, color: 'var(--ink-soft)', fontSize: 14.5 }}>
                  <span style={{ color: 'var(--creator-deep)', flexShrink: 0 }}><Check /></span>{t}
                </div>
              ))}
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {STEPS[key].map(([t, d], i) => (
                    <RevealItem key={t} style={{ display: 'flex', gap: 13 }}>
                      <div style={{ width: 26, height: 26, borderRadius: '50%', background: dotBg, color: dotInk, display: 'grid', placeItems: 'center', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 12.5, flexShrink: 0, marginTop: 1 }}>{i + 1}</div>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14.5 }}>{t}</div>
                        <div style={{ fontSize: 13, color: 'var(--ink-soft)', marginTop: 2, lineHeight: 1.45 }}>{d}</div>
                      </div>
                    </RevealItem>
                  ))}
                </div>
      ...
      <section className="lp-section lp-narrow">
        <Reveal style={{ textAlign: 'center', marginBottom: 'clamp(32px,4vw,48px)' }}>
          ...
          <h2 className="display-face" style={{ fontSize: 'clamp(26px,3.4vw,38px)', letterSpacing: '-0.02em' }}>A recommendation engine, not a list to scroll.</h2>
        </Reveal>
        ...
        <Reveal stagger className="resp-stats lp-narrow" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }}>
          {STATS.map(({ value, label, sub }) => (
            <RevealItem key={label} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--font-grotesk)', fontWeight: 700, fontSize: 'clamp(28px,3.5vw,38px)', letterSpacing: '-0.03em', color: '#fff', lineHeight: 1 }}>{value}</div>
              <div style={{ fontWeight: 700, fontSize: 13.5, color: '#fff', marginTop: 9 }}>{label}</div>
              <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,.5)', marginTop: 2 }}>{sub}</div>
            </RevealItem>
          ))}
        </Reveal>
      </section>
      ...
      <section style={{ background: 'var(--brand)', color: '#fff', padding: 'clamp(56px,8vw,88px) 20px', textAlign: 'center', borderTop: '1px solid rgba(255,255,255,.08)', position: 'relative', overflow: 'hidden' }}>
        ...
        <Reveal style={{ position: 'relative', zIndex: 1 }}>
          <h2 className="display-face" style={{ fontSize: 'clamp(28px,4.4vw,48px)', color: '#fff', marginBottom: 12, letterSpacing: '-0.03em' }}>
            Find your next collaboration.
          </h2>
          <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 16, marginBottom: 32 }}>
