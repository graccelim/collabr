...
export default function PlatformSwitcher({ platforms, active, onSelect }: { platforms: string[]; active: string; onSelect: (p: string) => void }) {
  return (
    <div className="pi-switch" style={{ display: 'inline-flex', maxWidth: '100%', overflowX: 'auto', scrollbarWidth: 'none', background: '#F1F5FC', border: '1px solid rgba(20,30,80,.09)', borderRadius: 11, padding: 4 }}>
      {platforms.map((p) => {
        const on = p === active
        ...
        return (
          <button key={p} type="button" onClick={() => onSelect(p)}
            style={{
              ...
              padding: '8px 15px', fontSize: 13.5, fontWeight: 600, flex: 'none', whiteSpace: 'nowrap',
