...
export default function PlatformInsights({ row }: { row: { platform: string; data: Data; ai_narrative: string | null } }) {
  ...
  // Forward-looking "next moves" — generated by the engine to be experiments to
  // try next, deliberately different from the "what's working" strengths below.
  const nextMoves: string[] = ((d as any).report?.nextMoves as string[] | undefined) ?? []
          ...
          {stats.map((s, i) => (
            <div key={i} className="pi-stat" style={{ flex: 1, minWidth: 0, padding: '17px 22px', borderLeft: i ? '1px solid rgba(14,16,22,.06)' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 11 }}>
                <span style={{ width: 30, height: 30, flex: 'none', borderRadius: 9, background: '#EEF1F8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><s.icon size={15} color="#2A3A8F" /></span>
                <StatDelta d={s.delta} />
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
            {nextMoves.slice(0, 4).map((m, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 14px', background: '#F7F8FC', border: '1px solid rgba(20,30,80,.07)', borderRadius: 11 }}>
                <span style={{ width: 7, height: 7, borderRadius: 999, background: '#2A3157', flex: 'none' }} />
                <span style={{ flex: 1, fontSize: 14, fontWeight: 500, color: '#0E1016' }}>{m}</span>
              </div>
            ))}
          </div>
        </div>
