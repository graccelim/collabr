    ...
    <div ref={ref}>
      <div className="plc-area" style={{ position: 'relative', height }}>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" preserveAspectRatio="none" style={{ display: 'block', position: 'absolute', inset: 0 }}>
          <defs>
            <linearGradient id="plc-area-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#2A3A8F" stopOpacity="0.16" /><stop offset="1" stopColor="#2A3A8F" stopOpacity="0" /></linearGradient>
            <linearGradient id="plc-line-grad" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#2A3A8F" /><stop offset="1" stopColor="#2A3A8F" /></linearGradient>
          </defs>
          ...
          <line x1={peak.x} x2={peak.x} y1={peak.y} y2={H - padBot} stroke="rgba(42,58,143,.35)" strokeDasharray="3 3" strokeWidth={1} vectorEffect="non-scaling-stroke" style={{ opacity: run ? 1 : 0, transition: 'opacity .4s ease .9s' }} />
        ...
        <span style={{ position: 'absolute', left: `${pX}%`, top: `${pY}%`, width: 9, height: 9, borderRadius: 999, background: '#fff', border: '2.5px solid #2A3A8F', transform: 'translate(-50%,-50%)', boxShadow: '0 2px 8px -2px rgba(42,58,143,.6)', opacity: run ? 1 : 0, transition: 'opacity .3s ease 1s' }} />
      </div>
      <div style={{ display: 'flex', marginTop: 9 }}>
        {data.map((d, i) => <span key={i} style={{ flex: 1, textAlign: 'center', fontFamily: MONO, fontSize: 11, fontWeight: i === peakIdx ? 600 : 500, color: i === peakIdx ? '#2A3A8F' : '#A2A8B6' }}>{d.label}</span>)}
      </div>
