...
// Shared pricing panel: Pro + Plus side by side, Monthly first. Animated — count-up
// prices, sliding toggle, sheen on the Plus CTA, rise-in. Beta: Pro = Free (struck
// S$29), Plus = full price struck → 50% off. Used inline on the Discover gate (over
// the blurred roster) and inside the billing modal (with onClose).
...
function Price({ amount, free = false, struck, period, note }: {
  amount: number; free?: boolean; struck?: string; period?: string; note?: string
}) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontFamily: 'var(--font-money)', fontWeight: 800, fontSize: 30, letterSpacing: '-.03em', color: '#0E1016', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
          {free ? 'Free' : `${CURRENCY}${v}`}
        </span>
  ...
  return (
    <div className="resp-1col" style={{
      ...
      <div style={{ position: 'relative', padding: '30px 28px' }}>
        {onClose && (
          <button aria-label="Close" onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, width: 32, height: 32, borderRadius: 999, border: '1px solid rgba(14,16,22,.12)', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
        ...
        {/* toggle — Annual | Monthly, Monthly selected by default, sliding indicator */}
        <div style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#F1F5FC', border: '1px solid rgba(14,16,22,.07)', borderRadius: 12, padding: 5, marginBottom: 18 }}>
          <span style={{ position: 'absolute', top: 5, left: 5, width: 'calc(50% - 5px)', height: 'calc(100% - 10px)', background: '#fff', border: '1px solid rgba(14,16,22,.1)', borderRadius: 9, boxShadow: '0 2px 6px -2px rgba(14,16,22,.16)', transform: cycle === 'monthly' ? 'translateX(100%)' : 'translateX(0)', transition: 'transform .34s cubic-bezier(.16,1,.3,1)' }} />
          {(['annual', 'monthly'] as const).map((c) => (
            <button key={c} type="button" onClick={() => setCycle(c)} style={{ position: 'relative', zIndex: 1, border: 'none', background: 'transparent', cursor: 'pointer', padding: '7px 6px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
              <span style={{ fontSize: 13.5, fontWeight: 600, color: cycle === c ? '#0E1016' : '#8A909C' }}>{c === 'annual' ? 'Annual' : 'Monthly'}</span>
              <span style={{ fontSize: 11, fontWeight: 600, color: c === 'annual' ? '#157A55' : '#B4B9C4' }}>{c === 'annual' ? '2 months free' : 'billed monthly'}</span>
            </button>
          ))}
        </div>
